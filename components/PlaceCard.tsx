...
export default function PlaceCard({place,compact=false}:{place:Place;compact?:boolean}){
 ...
 return <Animated.View {...webHover} style={[s.card,compact&&s.compact,glassSurface(true),{transform:[{scale},{translateY:lift.interpolate({inputRange:[0,1],outputRange:[0,-6]})}]}]}>
  <Pressable
  ...
  <Animated.View style={[s.heartWrap,{transform:[{scale:heartScale}]}]}>
   <Pressable style={[s.heart,glassSurface()]} onPress={toggleWish} hitSlop={8} accessibilityRole="button" accessibilityLabel={wish?'นำออกจากอยากไป':'เพิ่มในอยากไป'}>
    <Ionicons name={wish?'heart':'heart-outline'} size={19} color={wish?GLASS.gold:GLASS.white}/>
   </Pressable>
  </Animated.View>
...
const s=StyleSheet.create({
 card:{height:244,borderRadius:GLASS_RADIUS.lg,overflow:'hidden',backgroundColor:'rgba(4,80,96,.18)',borderWidth:1,borderColor:'rgba(255,255,255,.28)'},compact:{height:198},
 ...
 heartWrap:{position:'absolute',right:13,top:13},heart:{width:38,height:38,borderRadius:19,alignItems:'center',justifyContent:'center'},
 ...
 info:{position:'absolute',left:20,right:20,bottom:18},province:{fontSize:9,fontWeight:'900',letterSpacing:.7,color:'rgba(255,255,255,.72)'},title:{color:GLASS.white,fontSize:19,fontWeight:'900',marginTop:2,letterSpacing:-.25},meta:{flexDirection:'row',gap:7,marginTop:8,flexWrap:'wrap'},
 ...
 category:{backgroundColor:'rgba(255,255,255,.14)',color:GLASS.white,fontWeight:'900',fontSize:10,paddingHorizontal:8,paddingVertical:4,borderRadius:999,borderWidth:1,borderColor:'rgba(255,255,255,.16)'},
});
