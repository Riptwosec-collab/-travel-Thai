...
function TripCard({trip,onEdit,onDelete}:{trip:Trip;onEdit:()=>void;onDelete:()=>void}){
  ...
  return <View style={s.tripCard}>
    ...
    {open&&<View style={s.preview}>
      {(trip.days||[]).map(day=><View key={day.day} style={s.previewDay}>
        <View style={s.previewNo}><Text style={s.previewNoText}>{day.day}</Text></View>
        <View style={s.flex}>
          <Text style={s.previewTitle}>{day.title||`DAY ${day.day}`}</Text>
          {!!day.route&&<Text style={s.previewRoute}>เส้นทาง: {day.route}</Text>}
          {(day.schedule||[]).map((x,i)=><View key={x.id||i} style={s.previewSlotCard}>
            <View style={s.previewSlotMain}><Text style={s.previewTime}>{x.time||'--:--'}</Text><Text style={s.previewSlotText}>{x.title||'กิจกรรม'}</Text></View>
            {!!x.detail&&<Text style={s.previewDetail}>{x.detail}</Text>}
            {!!x.activities?.length&&<View style={s.previewActivityBox}>{x.activities.map((a,j)=><Text key={j} style={s.previewActivity}>• {a}</Text>)}</View>}
            {!!x.notes?.length&&<View style={s.previewNoteBox}>{x.notes.map((n,j)=><Text key={j} style={s.previewNote}>⚠ {n}</Text>)}</View>}
          </View>)}
          {!!day.accommodation&&<View style={s.previewMetaBox}><Ionicons name="bed-outline" size={14} color={COLORS.primary}/><Text style={s.previewMetaText}>ที่พัก: {day.accommodation}</Text></View>}
          {!!day.budgetItems?.length&&<View style={s.previewBudgetBox}><Text style={s.previewSectionLabel}>งบรายวัน</Text>{day.budgetItems.map((b,i)=><Text key={i} style={s.previewBudgetText}>• {b.text||`${b.label}: ${(b.min||0).toLocaleString()}–${(b.max||b.min||0).toLocaleString()} บาท`}</Text>)}</View>}
        </View>
      </View>)}
      {!!trip.budgetSummaryLines?.length&&<View style={s.previewSummaryBox}><Text style={s.previewSectionLabel}>งบรวมทั้งทริป</Text>{trip.budgetSummaryLines.map((line,i)=><Text key={i} style={s.previewSummaryText}>• {line}</Text>)}</View>}
      {!!trip.importantNotes?.length&&<View style={s.previewWarningBox}><Text style={s.previewSectionLabel}>หมายเหตุ / ข้อควรระวัง</Text>{trip.importantNotes.map((line,i)=><Text key={i} style={s.previewWarningText}>⚠ {line}</Text>)}</View>}
    </View>}
...
const s=StyleSheet.create({
  ...
  preview:{gap:10,paddingTop:8,borderTopWidth:1,borderTopColor:'rgba(7,61,75,.07)'},previewDay:{flexDirection:'row',gap:8,paddingTop:4},previewNo:{width:32,height:32,borderRadius:10,backgroundColor:COLORS.dark,alignItems:'center',justifyContent:'center'},previewNoText:{fontSize:10,fontWeight:'900',color:'#fff'},previewTitle:{fontSize:12.5,fontWeight:'900',color:COLORS.text,marginBottom:4},previewRoute:{fontSize:9.5,lineHeight:15,fontWeight:'700',color:COLORS.textMuted,marginBottom:5},previewSlotCard:{padding:8,borderRadius:12,backgroundColor:'rgba(248,252,252,.86)',borderWidth:1,borderColor:'rgba(7,61,75,.07)',gap:5,marginBottom:6},previewSlotMain:{flexDirection:'row',gap:7,alignItems:'flex-start'},previewTime:{width:68,fontSize:9.5,fontWeight:'900',color:COLORS.primary},previewSlotText:{flex:1,fontSize:10.5,lineHeight:15,fontWeight:'800',color:COLORS.text},previewDetail:{fontSize:9.5,lineHeight:15,color:COLORS.textMuted},previewActivityBox:{gap:2},previewActivity:{fontSize:9.5,lineHeight:15,color:COLORS.text},previewNoteBox:{padding:7,borderRadius:9,backgroundColor:'rgba(242,211,154,.14)',gap:2},previewNote:{fontSize:9.5,lineHeight:15,fontWeight:'700',color:COLORS.text},previewMetaBox:{padding:8,borderRadius:10,backgroundColor:'rgba(232,246,246,.82)',flexDirection:'row',alignItems:'flex-start',gap:6,marginTop:4},previewMetaText:{flex:1,fontSize:9.5,lineHeight:15,fontWeight:'800',color:COLORS.text},previewBudgetBox:{padding:8,borderRadius:10,backgroundColor:'rgba(255,255,255,.76)',gap:3,marginTop:5},previewSectionLabel:{fontSize:10.5,fontWeight:'900',color:COLORS.text,marginBottom:2},previewBudgetText:{fontSize:9.5,lineHeight:15,color:COLORS.textMuted},previewSummaryBox:{padding:10,borderRadius:12,backgroundColor:'rgba(232,246,246,.86)',gap:3},previewSummaryText:{fontSize:9.5,lineHeight:15,color:COLORS.text},previewWarningBox:{padding:10,borderRadius:12,backgroundColor:'rgba(242,211,154,.15)',gap:3},previewWarningText:{fontSize:9.5,lineHeight:15,fontWeight:'700',color:COLORS.text},
});
