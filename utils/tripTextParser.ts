...
const parseDay=(dayNo:number,rawBody:string):TripDay=>{
  ...
  const sourceLines=usefulLines(body);
  const title=sourceLines[0]||`DAY ${dayNo}`;
  const work=sourceLines.slice(1);
...
const parseAccommodationPlan=(value:string):TripAccommodationNight[]=>{
  const sourceLines=usefulLines(value);
  const result:TripAccommodationNight[]=[];
  for(let i=0;i<sourceLines.length;i++){
    const m=sourceLines[i].match(/^คืนที่\s*(\d+)\s*:?\s*(.*)$/i);
    if(!m)continue;
    let location=m[2].trim();
    if(!location&&sourceLines[i+1]&&!/^คืนที่/i.test(sourceLines[i+1]))location=sourceLines[++i].trim();
...
const parseBudgetTiers=(sourceLines:string[]):TripBudgetTier[]=>{
  ...
  sourceLines.forEach((line,index)=>{
    ...
    if(/^ประมาณ/.test(label)&&index>0)label=sourceLines[index-1].replace(/:$/,'').trim();
...
export const parseDetailedTripText=(raw:string):ParsedTripText=>{
  ...
  const title=sourceLines.find(x=>/^แผนเที่ยว/i.test(x))||sourceLines[0];

  const budgetLine=sourceLines.find(x=>/^งบประมาณสำหรับ/i.test(x));
  ...
  const transportLine=sourceLines.find(x=>/^รูปแบบการเดินทาง\s*:/i.test(x));
